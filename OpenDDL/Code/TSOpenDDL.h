...
/// @file TSOpenDDL.h
/// @brief Contains the definitions for the OpenDDL data structures and processing functions.
...
namespace Terathon
{
    ...
    enum : DataResult
    {
        ...
        kDataExtraneousSubstructure = 'EXSB', ///< A structure contains too many
                                              ///< substructures of a legal type.
        kDataInvalidDataFormat = 'IVDF',      ///< The primitive data contained in a structure uses an invalid
                                              ///< format (type, element count, subarray size, or state data).
    ...
    /// @class Structure
    /// @brief Represents a data structure in an OpenDDL file.
    /// The @ref Structure class is the base class for objects that represent data
    /// structures in an Open Data Description Language (OpenDDL) file. Structures
    /// of a specified data type are represented by objects derived from the @ref
    /// Structure class.
    class Structure : public Tree<Structure>
    {
    ...
    private:
        ...
        std::unordered_map<KeyType, Structure*> structureMap;

        const char* textLocation;

    protected:
        /// @brief Constructor for the base Structure class.
        /// @param type The type of the structure.
        explicit Structure(StructureType type);

        /// @brief Sets the base structure type.
        /// @param type The base structure type to set.
    ...
    public:
        ...
        /// @brief Returns the key type used to identify the structure in map collections.
        /// @return The key type, which is the structure name.
        KeyType GetKey(void) const
        {
            return (structureName);
        }

        /// @brief Returns the type of the structure.
        ...
        /// @brief Returns the base structure type.
        /// @return The base structure type.
        ...
        void SetGlobalNameFlag(bool flag)
        {
            globalNameFlag = flag;
        }
        ...
        /// @brief Finds a named structure using a local reference.
        /// @param [in] reference The structure reference to search for.
        /// @param [in] index The index of the structure name to search for (default is 0).
        /// @return A pointer to the found structure, or nullptr if not found.
        ///
        /// This @ref FindStructure function finds a structure
        Structure* FindStructure(const StructureRef& reference, int32 index = 0) const;

        /// @brief Validates a property against the data description.
        /// @param [in] dataDescription The data description to validate against.
        /// @param [in] identifier The identifier of the property to validate.
        /// @param [out] type The data type of the property.
        /// @param [out] value The value of the property.
        /// @return True if the property is valid, false otherwise.
        ///
        /// The @ref ValidateProperty function is called for each property specified in an OpenDDL file for a particular data structure to determine
        /// whether the property is valid, and if so, what type it expects and where to store the value. This function should be overriden by an subclasses
        /// of @ref Structure that define their own properties.
        virtual bool ValidateProperty(const DataDescription* dataDescription, std::string_view identifier, DataType* type, void** value);

        /// @brief Validates a substructure against the data description.
        /// @param [in] dataDescription The data description to validate against.
        /// @param [in] structure The structure to validate.
        /// @return True if the substructure is valid, false otherwise.
        ///
        /// The @ref ValidateSubstructure function is called for the @ref Structure object representing the enclosing data
        /// structure each time a new substructure is created to determine whether the new substructure can legally
        /// be contained in the data of the @ref Structure object. An overriding implementation should examine the
        /// structure specified by the @a structure parameter and return @a true if it can legally appear as a direct
        /// subnode of the @ref Structure object for which the @ref ValidateSubstructure function is called. Otherwise, the
        /// function should return @a false.
        virtual bool ValidateSubstructure(const DataDescription* dataDescription, const Structure* structure) const;

        virtual bool       GetStateValue(std::string_view identifier, uint32* state) const;
        virtual DataResult ProcessData(DataDescription* dataDescription);
    };

    /// \class	PrimitiveStructure		Base class for built-in primitive data
    /// structures in an OpenDDL file.
    ///
    /// The $PrimitiveStructure$ class is the base class for built-in primitive data
    /// structures in an OpenDDL file.
    ///
    ...
    ///
    /// \desc
    /// The $PrimitiveStructure$ class is the base class for all objects that
    /// represent built-in primitive data structures in an Open Data Description
    /// Language (OpenDDL) file. Specific types of primitive data structures are
    /// represented by specializations of the $@DataStructure@$ class template.
    ///
    /// The base structure type returned by the $@Structure::GetBaseStructureType@$
    /// function for all built-in primitive data structures is
    /// $kStructurePrimitive$.
    ///
    /// \base	Structure		A primitive structure is a special type of
    /// $Structure$ object.
    ///
    /// \also	$@DataStructure@$

    /// \function	PrimitiveStructure::GetArraySize		Returns the array size
    /// for a primitive data type.
    ///
    /// \proto	uint32 GetArraySize(void) const;
    ///
    /// \desc
    /// The $GetArraySize$ function returns the number of elements in each subarray
    /// contained in the data belonging to a primitive data structure. This
    /// corresponds to the number specified in brackets after the type identifier.
    /// If no array size is specified for a structure, then the $GetArraySize$
    /// function returns zero.
    ///
    /// \also	$@DataStructure::GetArrayDataElement@$

    /// \function	PrimitiveStructure::GetStateFlag		Returns a boolean value
    /// indicating whether state data is present.
    ///
    /// \proto	bool GetStateFlag(void) const;
    ///
    /// \desc
    /// The $GetStateFlag$ function returns $true$ if the primitive data structure
    /// contains state data and $false$ if it does not. State data can exist only
    /// for primitive data that has subarrays, so the $GetStateFlag$ function always
    /// returns $false$ if the $@PrimitiveStructure::GetArraySize@$ function returns
    /// zero.
    ///
    ...
    /// \class	DataStructure		Represents a specific built-in primitive data
    /// structure in an OpenDDL file.
    ///
    /// The $DataStructure$ class template represents each of the specific built-in
    /// primitive data structure in an OpenDDL file.
    ///
    /// \def	template <class type> class DataStructure final : public
    /// PrimitiveStructure
    ///
    /// \tparam		type	An object type representing the specific type of data
    /// contained in the structure.
    ///
    /// \ctor	DataStructure();
    ///
    /// \desc
    /// A specialization of the $DataStructure$ class template represents each of
    /// the built-in primitive data structures in an Open Data Description Language
    /// (OpenDDL) file. The $type$ template parameter can only be one of the
    /// following types.
    ///
    /// \value	BoolDataType		A boolean type that can have the value $true$ or
    /// $false$.
    /// \value	Int8DataType		An 8-bit signed integer that can have values in
    /// the range
    /// [&minus;2<sup>7</sup>,&#x202F;2<sup>7</sup>&#x202F;&mins;&#x202F;1].
    /// \value	Int16DataType		A 16-bit signed integer that can have values in
    /// the range
    /// [&minus;2<sup>15</sup>,&#x202F;2<sup>15</sup>&#x202F;&mins;&#x202F;1].
    /// \value	Int32DataType		A 32-bit signed integer that can have values in
    /// the range
    /// [&minus;2<sup>31</sup>,&#x202F;2<sup>31</sup>&#x202F;&mins;&#x202F;1].
    /// \value	Int64DataType		A 64-bit signed integer that can have values in
    /// the range
    /// [&minus;2<sup>63</sup>,&#x202F;2<sup>63</sup>&#x202F;&mins;&#x202F;1].
    /// \value	UInt8DataType		An 8-bit unsigned integer that can have values
    /// in the range [0,&#x202F;2<sup>8&#x202F;-&#x202F;1].
    /// \value	UInt16DataType		A 16-bit unsigned integer that can have values
    /// in the range [0,&#x202F;2<sup>16&#x202F;-&#x202F;1].
    /// \value	UInt32DataType		A 32-bit unsigned integer that can have values
    /// in the range [0,&#x202F;2<sup>32&#x202F;-&#x202F;1].
    /// \value	UInt64DataType		A 64-bit unsigned integer that can have values
    /// in the range [0,&#x202F;2<sup>64&#x202F;-&#x202F;1].
    /// \value	HalfDataType		A 16-bit floating-point type conforming to the
    /// standard S1E5M10 format.
    /// \value	FloatDataType		A 32-bit floating-point type conforming to the
    /// standard S1E8M23 format.
    /// \value	DoubleDataType		A 64-bit floating-point type conforming to the
    /// standard S1E11M52 format.
    /// \value	StringDataType		A double-quoted character string with contents
    /// encoded in UTF-8.
    /// \value	RefDataType			A sequence of structure names, or the keyword
    /// $null$.
    /// \value	TypeDataType		A type whose values are identifiers naming types
    /// in the first column of this table.
    /// \value	Base64Type			A raw binary data type with contents encoded as
    /// base64.
    ///
    /// \desc
    /// The raw data belonging to a data structure is stored as a linear array in
    /// memory, regardless of whether subarrays are specified. The total number of
    /// data elements present can be retrieved with the
    /// $@DataStructure::GetDataElementCount@$ function, and the data for each
    /// element can be retrieved with the $@DataStructure::GetDataElement@$
    /// function. If subarrays are in use, then the elements belonging to each
    /// subarray are stored contiguously, and each subarray is then stored
    /// contiguously with the one preceding it. The
    /// $@DataStructure::GetArrayDataElement@$ function can be used to retrieve a
    /// pointer to the beginning of a specific subarray.
    ///
    /// \base	PrimitiveStructure		Each data structure specialization is a
    /// specific type of $PrimitiveStructure$ object.

    /// \function	DataStructure::GetDataElementCount		Returns the total number
    /// of data elements stored in a data structure.
    ///
    /// \proto	int32 GetDataElementCount(void) const;
    ///
    /// \desc
    /// The $GetDataElementCount$ function returns the total number of individual
    /// primitive data elements stored in a data structure. If a data structure
    /// contains subarrays, then this function returns the number of elements in
    ...
    /// \function	DataStructure::GetDataElement		Returns a single data
    ...
    ///
    /// \desc
    /// The $GetDataElement$ function returns a single primitive data element stored
    /// in a data structure. The legal values of the $index$ parameter range from
    /// zero to <i>n</i>&nbsp;&minus;1, inclusive, where <i>n</i> is the total count
    /// of data elements returned by the $@DataStructure::GetDataElementCount@$
    /// function.
    ///
    /// The $PrimType$ type is defined by the class corresponding to the $type$
    /// template parameter associated with the particular specialization of the
    /// $DataStructure$ class template.
    ///
    /// \also	$@DataStructure::GetArrayDataElement@$
    /// \also	$@DataStructure::GetDataElementCount@$
    /// \also	$@PrimitiveStructure::GetArraySize@$

    /// \function	DataStructure::GetArrayDataElement		Returns a pointer to a
    /// subarray stored in a data structure.
    ///
    ...
    ///
    /// \desc
    /// The $GetArrayDataElement$ function returns a pointer to the first element in
    /// a subarray stored in a data structure. The legal values of the $index$
    /// parameter range from zero to <i>n</i>&nbsp;/&nbsp;<i>s</i>&nbsp;&minus;1,
    /// inclusive, where <i>n</i> is the total count of data elements returned by
    /// the $@DataStructure::GetDataElementCount@$ function, and <i>s</i> is the
    /// size of each subarray returned by the $@PrimitiveStructure::GetArraySize@$
    /// function. The elements of each subarray are stored contiguously in memory.
    ///
    /// The $PrimType$ type is defined by the class corresponding to the $type$
    /// template parameter associated with the particular specialization of the
    /// $DataStructure$ class template.
    ...
    /// \function	DataStructure::GetArrayStateElement		Returns the state
    /// associated with a single subarray in a data structure.
    ///
    /// \proto	const uint32& GetArrayStateElement(int32 index) const;
    ///
    /// \param	index	The zero-based index of the subarray for which to retrieve
    /// the state.
    ///
    /// \desc
    /// The $GetArrayStateElement$ function returns a reference to the state
    /// associated with a single subarray in a data structure. The legal values of
    /// the $index$ parameter range from zero to
    /// <i>n</i>&nbsp;/&nbsp;<i>s</i>&nbsp;&minus;1, inclusive, where <i>n</i> is
    /// the total count of data elements returned by the
    /// $@DataStructure::GetDataElementCount@$ function, and <i>s</i> is the size of
    /// each subarray returned by the $@PrimitiveStructure::GetArraySize@$ function.
    /// States are stored contiguously in memory.
    ///
    /// State data exists only if the $@PrimitiveStructure::GetStateFlag@$ function
    /// returns $true$, and there is one state value per subarray. The
    /// $GetArrayStateElement$ function should not be called if the
    /// $PrimitiveStructure::GetStateFlag$ function returns $false$.
    ///
    ...
    public:
        DataStructure();
        DataStructure(uint32 size, bool state = false);
        ~DataStructure();
    ...
    /// \class	DataDescription		Represents a derivative file format based on the
    /// OpenDDL language.
    ///
    /// The $DataDescription$ class represents a derivative file format based on the
    /// OpenDDL language.
    ///
    ...
    ///
    /// \desc
    /// The $DataDescription$ class is the base class for objects that represent
    /// derivative file format based on the Open Data Description Language
    /// (OpenDDL). It serves as a container for the tree hierarchy of data
    /// structures in an OpenDDL file.
    ///
    /// A subclass of the $DataDescription$ class represents a specific
    /// OpenDDL-based file format and provides the means for constructing custom
    /// $@Structure@$ subclasses by overriding the
    /// $@DataDescription::CreateStructure@$ function.
    ///
    /// \also	$@Structure@$
    /// \also	$@PrimitiveStructure@$
    /// \also	$@DataStructure@$
    ...
    /// \function	DataDescription::GetRootStructure		Returns root structure
    /// for an OpenDDL file.
    ///
    /// \proto	const Structure *GetRootStructure(void) const;
    ///
    /// \desc
    /// The $GetRootStructure$ function returns the root structure for an OpenDDL
    /// file. The direct subnodes of the root structure correspond to the top-level
    ...
    ///
    /// \desc
    /// The $FindStructure$ function finds the structure referenced by the sequence
    /// of names stored in the $reference$ parameter and returns a pointer to it. If
    /// no such structure exists, then the return value is $nullptr$.
    ///
    /// The reference must be a global reference, meaning that the first name stored
    /// in the reference is a global name as indicated by a value of $true$ being
    /// returned by the $@StructureRef::GetGlobalRefFlag@$ function. If the
    /// reference is not a global reference, then this function always returns
    /// $nullptr$. The $@Structure::FindStructure@$ function should be used to find
    /// a structure through a local reference.
    ///
    /// If the specified reference has an empty name array, then the return value is
    /// always $nullptr$. The empty name array is assigned to a reference data value
    /// when $null$ appears in the OpenDDL file.
    ///
    /// \also	$@StructureRef@$
    ...
    /// \function	DataDescription::CreateStructure		Creates a custom data
    /// structure.
    ///
    /// \proto	virtual Structure *CreateStructure(const String<>& identifier)
    /// const;
    ///
    /// \param	identifier		The identifier of a data structure in an OpenDDL
    /// file.
    ///
    /// \desc
    /// The $CreateStructure$ function should be overridden by any subclass of the
    /// $DataDescription$ class representing a file format that defines custom data
    /// structures. The implementation should use the $new$ operator to create a new
    /// object based on the $Structure$ subclass corresponding to the $identifier$
    /// parameter. If the identifier is not recognized, then this function should
    /// return $nullptr$. The default implementation always returns $nullptr$.
    ///
    /// \also	$@Structure@$

    /// \function	DataDescription::ValidateTopLevelStructure		Determines the
    /// validity of a top-level structure.
    ///
    /// \proto	virtual bool ValidateTopLevelStructure(const Structure *structure)
    /// const;
    ///
    /// \param	structure		The top-level structure to validate.
    ///
    /// \desc
    /// The $ValidateTopLevelStructure$ function is called each time a new structure
    /// is created at the top level of an OpenDDL file to determine whether the new
    /// structure can legally appear outside all other structures. An overriding
    /// implementation should examine the structure specified by the $structure$
    /// parameter and return $true$ if it can legally appear at the top level of a
    /// file, and it should return $false$ otherwise.
    ///
    /// An implementation would typically call the $@Structure::GetStructureType@$
    /// function to make its decision, but other information such as the base
    /// structure type or the primitive subarray size may also be taken into
    /// account. At the time that the $ValidateTopLevelStructure$ function is
    /// called, no data belonging to the structure is available, so the data itself
    /// cannot be used to validate any top-level structures.
    ///
    /// The default implementation of the $ValidateTopLevelStructure$ function
    /// always returns $true$.
    ///
    /// \also	$@Structure::ValidateSubstructure@$

    /// \function	DataDescription::ProcessText		Parses an OpenDDL file and
    /// processes the top-level data structures.
    ///
    /// \proto	DataResult ProcessText(const char *text);
    ///
    /// \param	text	The full contents of an OpenDDL file with a terminating zero
    /// byte.
    ///
    /// \desc
    /// The $ProcessText$ function parses the entire OpenDDL file specified by the
    /// $text$ parameter. If the file is successfully parsed, then the data is
    /// processed as described below. If an error occurs during the parsing stage,
    /// then the $ProcessText$ function returns one of the following values, and the
    /// $DataDescription$ object contains no data.
    ///
    /// \table	DataResult
    ///
    /// During the parsing stage, the $@DataDescription::CreateStructure@$ function
    /// is called for each custom data structure that is encountered in order to
    /// construct an object whose type is the proper subclass of the $@Structure@$
    /// class.
    ///
    /// After a successful parse, the $ProcessText$ function iterates through all of
    /// the top-level data structures in the file (which are the direct subnodes of
    /// the root structure returned by the $@DataDescription::GetRootStructure@$
    /// function) and calls the $@Structure::ProcessData@$ function for each one. If
    /// an error is returned by any of the calls to the $@Structure::ProcessData@$
    /// function, then the processing stops, and the same error is returned by the
    /// $DataDescription::ProcessText$ function. If all of the top-level data
    /// structures are processed without error, then the
    /// $DataDescription::ProcessText$ function returns $kDataOkay$. The error
    /// returned during the processing stage can be one of the following values or a
    /// value defined by a derivative data format.
    ///
    /// \table	DataProcessResult
    ///
    /// If an error is returned for either the parsing stage or the processing
    /// stage, then the line number where the error occurred can be retrieved by
    /// calling the $@DataDescription::GetErrorLine@$ function.
    ///
    /// The default implementation of the $@Structure::ProcessData@$ function
    /// iterates over the direct subnodes of a data structure and calls the
    /// $ProcessData$ function for each one. If all overrides call the base class
    /// implementation, then the entire tree of data structures will be visited
    /// during the processing stage.
    ///
    /// Any implementation of the $@Structure::ProcessData@$ function may make the
    /// following assumptions about the data:
    ///
    /// 1. The input text is syntactically valid.<br/>
    /// 2. If an unrecognized structure was encountered in the input text, then it
    /// was removed along with all of its substructures.<br/>
    /// 3. Each recognized structure is valid as indicated by the
    /// $@Structure::ValidateSubstructure@$ function called for its enclosing
    /// structure.<br/>
    /// 4. If a property was valid as indicated by the
    /// $@Structure::ValidateProperty@$ function called for the associated
    /// structure, then it has a value of the proper type assigned to it.
    /// Unrecognized properties were ignored.<br/>
    /// 5. Any existing subarrays of primitive data have the correct number of
    /// elements, matching the number specified in brackets after the primitive type
    /// identifier.<br/>
    /// 6. Any existing state identifiers associated with primitive data subarrays
    /// are valid as indicated by the $@Structure::GetStateValue@$ function.
    ///
    /// \also	$@Structure::ProcessData@$
    /// \also	$@DataDescription::GetErrorLine@$

    /// \function	DataDescription::GetErrorLine		Returns the line on which an
    /// error occurred.
    ///
    /// \proto	int32 GetErrorLine(void) const;
    ///
    /// \desc
    /// The $GetErrorLine$ function returns the line number on which an error
    /// occurred when the $@DataDescription::ProcessText@$ function was called. Line
    /// numbering begins at one. If the $@DataDescription::ProcessText@$ function
    /// returned $kDataOkay$, then the $GetErrorLine$ function will return zero.
    ///
    /// \also	$@DataDescription::ProcessText@$
    ...
    private:
        std::unordered_map<Structure::KeyType, Structure*> structureMap;
    ...
    public:
        ...
        std::unordered_map<Structure::KeyType, Structure*>* GetStructureMap(void)
        {
            return (&structureMap);
        }
